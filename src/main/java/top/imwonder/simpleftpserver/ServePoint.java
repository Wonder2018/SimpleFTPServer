...
public class ServePoint extends Thread {
    ...
    String dir = "/home/wonder2019";// 当前目录
    String rootdir = "/home/wonder2019"; // 默认根目录,在checkPASS中设置
    FTPState state; // 用户状态标识符,在checkPASS中设置
    String reply; // 返回报告
    PrintWriter ctrlOutput;
    FTPOption type; // 文件类型(ascII 或 bin)
    String requestfile = "";
    boolean isrest = false;

    // FtpHandler方法
    // 构造方法
    public ServePoint(Socket s, int i) {
        ctrlSocket = s;
        id = i;
    }
    ...
    boolean checkPASS(String s) {
        ...
        // }
        // }
        System.out.println("jump out pwdchk!");
        return true;
    }
        ...
        if (f.isDirectory() && f.exists()) {
            if (param.equals("..") || param.equals("..\\")) {
                if (dir.compareToIgnoreCase(rootdir) == 0) {
                    ...
                    if (s1 != null) {
                        dir = s1;
                        reply = "250 请求的文件处理结束, 当前目录变为: " + dir + "\r";
                    } else
                        reply = "550 此路径不存在\r";
                }
                ...
                dir = param;
                reply = "250 请求的文件处理结束, 工作路径变为 " + dir + "\r";
            }
     ...
     * 处理这个命令时，先创建一个临时的套接字向客户端发送目录信息。这个套接字的目的 端口号缺省为，然后为当前工作目录创建File
     * 对象，利用该对象的list()方法得到一 个包含该目录下所有文件和子目录名称的字符串数组，然后根据名称中是否含有文件名
    ...
    boolean commandLIST()// 文件和目录的列表
    {
        try {
            ...
            dataSocket = new Socket(remoteHost, remotePort, ctrlSocket.getLocalAddress(), (int)(Math.random()*1000));
            PrintWriter dout = new PrintWriter(dataSocket.getOutputStream(), true);
            if (param.equals("-a") || param.equals("LIST")) {
                ctrlOutput.println("150 文件状态正常,ls以 ASCII 方式操作\r");
                ctrlOutput.flush();
                ...
                // dout.println(fileType + dirStructure[i] + "\r");//
            ...
            dout.close();
            dataSocket.close();
            reply = "226 传输数据连接结束\r";
        } catch (Exception e) {
            e.printStackTrace();
            reply = "451 Requested action aborted: local error in processing\r";
            return false;
        }
    ...
    {
        if (param.equals("A")) {
            type = FTPOption.FTYPE_ASCII;// 0
            reply = "200 命令正确 ,转 ASCII 模式\r";
        } else if (param.equals("I")) {
            type = FTPOption.FTYPE_BIN;// 1
            reply = "200 命令正确 转 BINARY 模式\r";
        } else
            reply = "504 命令不能执行这种参数\r";
    ...
    boolean commandRETR() {
        ...
        if (isrest) {
            ...
            {
                try {
                    ctrlOutput.println("150 Opening ASCII mode data connection for " + requestfile + "\r");
    ...
    boolean commandSTOR() {
        ...
        {
            try {
                ctrlOutput.println("150 Opening Binary mode data connection for " + requestfile + "\r");
                dataSocket = new Socket(remoteHost, remotePort, InetAddress.getLocalHost(), 20);
    ...
    boolean commandDELE() {
        ...
        if (i == 0) {
            reply = "550 请求的动作未执行,文件不存在,或目录不对,或其他\r";
            return false;
        }
        if (i == 1) {
            File f = new File(param);
        ...
        reply = "250 请求的文件处理结束,成功删除服务器上文件\r";
        return false;
    ...
    boolean commandMKD() {
        ...
        if (s1.startsWith(s2)) {
            File f = new File(param);
            if (f.exists()) {
                reply = "550 请求的动作未执行,目录已存在\r";
                return false;
            } else {
                f.mkdirs();
                reply = "250 请求的文件处理结束, 目录建立\r";
            }
        } else {
            File f = new File(addTail(dir) + param);
            if (f.exists()) {
